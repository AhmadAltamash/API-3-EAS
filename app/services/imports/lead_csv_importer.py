...
class LeadCSVImporter:
    ...
    # Loose enough to catch "+1 (555) 123-4567", "555-123-4567", or
    # "5551234567", but requires at least 7 actual digits so it
    # doesn't false-positive on short numeric codes or IDs.
    PHONE_PATTERN = re.compile(
        r"^\+?[\d\s\-().]{7,20}$"
    )

    # This app only ever deals in US/Canada leads, so a small known-
    # token check is reliable here without needing a full country
    # gazetteer - broader than this would need a real list. Bare "CA"
    # deliberately excluded - it's also California's standard state
    # abbreviation, and including it would misdetect a State column
    # as Country on any US-state-abbreviation data.
    COUNTRY_TOKENS = {
    ...
    HEADER_SYNONYMS = {
        ...
        "website": {
            ...
            "website_url", "web_url", "site_url", "web_link",
            "website_link", "homepage", "company_website",
            "company_url", "web_address", "company_site"
        },
        "country": {
    ...
    def _match_header(self, header_row):
        ...
        # Safety net for a compound header not in the exact-match sets
        # above (this is exactly the bug that let "Website URL" -
        # normalized to "website_url" - through undetected: it wasn't
        # an exact match for any synonym, so the whole column was
        # silently dropped). Split on "_" and match whole WORD tokens
        # only, never a raw substring - "hourly_rate" contains the
        # letters "u","r","l" in sequence but does not contain the
        # whole word "url" as a token, so this stays precise instead
        # of colliding with unrelated headers.
        for idx, cell in enumerate(normalized):
    ...
    def _infer_columns(self, sample_rows):
        ...
        for c in range(num_cols):
            ...
            stats.append({
                ...
                "phone": sum(
                    1 for v in values
                    if self.PHONE_PATTERN.match(v)
                    and sum(ch.isdigit() for ch in v) >= 7
                ) / len(values),
                "country": sum(
                    1 for v in values if v.strip().lower() in self.COUNTRY_TOKENS
                ) / len(values)
            })
        ...
        # Phone and country checked AFTER email/website (a phone-shaped
        # value can't also be an email/website match, but check order
        # matters for tie-breaking on ambiguous columns) and BEFORE
        # picking "company", so a phone/country column never gets
        # silently misassigned as the company name.
        remaining_for_phone = [
            c for c in range(num_cols)
            if c != col_map.get("email") and c != col_map.get("website")
        ]

        if remaining_for_phone:

            phone_col = max(remaining_for_phone, key=lambda c: stats[c]["phone"])

            if stats[phone_col]["phone"] > 0.5:
                col_map["phone"] = phone_col

        remaining_for_country = [
            c for c in range(num_cols)
            if c not in (col_map.get("email"), col_map.get("website"), col_map.get("phone"))
        ]

        if remaining_for_country:

            country_col = max(remaining_for_country, key=lambda c: stats[c]["country"])

            if stats[country_col]["country"] > 0.5:
                col_map["country"] = country_col
        ...
        remaining = [
            ...
            and c != col_map.get("phone")
            and c != col_map.get("country")
        ]
